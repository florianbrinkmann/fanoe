 ...
 * Define Constants
 */
define('fanoe_SHORTNAME', 'fanoe');
define('fanoe_PAGE_BASENAME', 'fanoe-settings');

/*
 * Specify Hooks/Filters
 */
add_action( 'admin_menu', 'fanoe_add_menu' );
add_action( 'admin_init', 'fanoe_register_settings' );

/*
 * Include the required files
 */
// page settings sections & fields
require_once('fanoe-theme-options.php');
 ...
 * @return array
 */
function fanoe_get_settings() {
	
	$output = array();
	
	// put together the output array 
	$output['fanoe_option_name'] 		= 'fanoe_options';
	$output['fanoe_page_title'] 		= __( 'fanoe Settings Page','fanoe');
	$output['fanoe_page_sections'] 	= fanoe_options_page_sections();
	$output['fanoe_page_fields'] 		= fanoe_options_page_fields();
 ...
 */
function fanoe_create_settings_field( $args = array() ) {
	// default array to overwrite when calling the function
	$defaults = array(
		'id'      => 'default_field', 					// the ID of the setting in our options array, and the ID of the HTML form element
		'title'   => "__('Default field', 'fanoe')", 					// the label for the HTML form element
		'desc'    => "__('This is a default description.', 'fanoe')", 	// the description displayed under the HTML form element
		'std'     => '', 								// the default value for this setting
		'type'    => 'text', 							// the HTML form element to use
		'section' => 'main_section', 					// the section this setting belongs to — must match the array key of a section in fanoe_options_page_sections()
	...
	// additional arguments for use in form field output in the function fanoe_form_field_fn!
	$field_args = array(
		'type'      => $type,
		'id'        => $id,
		'desc'      => $desc,
	...
	add_settings_field( $id, $title, 'fanoe_form_field_fn', __FILE__, $section, $field_args );
 ...
 */
function fanoe_register_settings(){
	
	// get the settings sections array
	$settings_output 	= fanoe_get_settings();
	$fanoe_option_name = $settings_output['fanoe_option_name'];
	
	//setting
	// register_setting( $option_group, $option_name, $sanitize_callback );
	register_setting($fanoe_option_name, $fanoe_option_name, 'fanoe_validate_options' );
	
	//sections
	// add_settings_section( $id, $title, $callback, $page );
	if(!empty($settings_output['fanoe_page_sections'])){
		// call the "add_settings_section" for each!
		foreach ( $settings_output['fanoe_page_sections'] as $id => $title ) {
			add_settings_section( $id, $title, 'fanoe_section_fn', __FILE__);
		}
	}
	
	//fields
	if(!empty($settings_output['fanoe_page_fields'])){
		// call the "add_settings_field" for each!
		foreach ($settings_output['fanoe_page_fields'] as $option) {
			fanoe_create_settings_field($option);
		}
	}
}
 ...
 */
function fanoe_settings_scripts(){
	wp_enqueue_style('fanoe_theme_settings_css', get_template_directory_uri() . '/inc/fanoe_theme_settings.css');
	wp_enqueue_script( 'fanoe_theme_settings_js', get_template_directory_uri() . '/inc/fanoe_theme_settings.js', array('jquery'));
	wp_enqueue_script('jscolor.js', get_template_directory_uri() . '/js/colorpicker/jscolor.js'); //color picker for '#' value fields
}

/*
 * The Admin menu page
 */
function fanoe_add_menu(){
	
	$settings_output 		= fanoe_get_settings();
	
	// Display Settings Page link under the "Appearance" Admin Menu
	$fanoe_settings_page = add_theme_page(__('fanoe Options', 'fanoe'), __('fanoe Options','fanoe'), 'manage_options', fanoe_PAGE_BASENAME, 'fanoe_settings_page_fn');
		// css & js
		add_action( 'load-'. $fanoe_settings_page, 'fanoe_settings_scripts' );
		
		add_action( 'load-'. $fanoe_settings_page, 'fanoe_theme_options_add_help' );
}
 ...
 */
function  fanoe_section_fn($desc) {
	echo "";
}
 ...
 * @return echoes output
 */
function fanoe_form_field_fn($args = array()) {
	...
	$fanoe_option_name = $settings_output['fanoe_option_name'];
	$options 			= get_option($fanoe_option_name);
	...
	switch ( $type ) {
		case 'text':
			...
			echo "<input class='regular-text $class' type='text' id='$id' name='" . $fanoe_option_name . "[$id]' value='$options[$id]' />";
		...
		case "multi-text":
			foreach($choices as $item) {
				...
				echo "<span>$item[0]:</span> <input class='$field_class' type='text' id='$id|$item[1]' name='" . $fanoe_option_name . "[$id|$item[1]]' value='$value' /><br/>";
			}
		...
		case 'textarea':
			...
			echo "<textarea class='textarea$field_class' type='text' id='$id' name='" . $fanoe_option_name . "[$id]' rows='10' cols='100'>$options[$id]</textarea>";
			echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : ""; 		
		break;
		
		case 'select':
			echo "<select id='$id' class='select$field_class' name='" . $fanoe_option_name . "[$id]'>";
				foreach($choices as $item) {
					$value 	= esc_attr($item, 'fanoe');
					$item 	= esc_html($item, 'fanoe');
		...
		case 'checkbox':
			echo "<input class='checkbox$field_class' type='checkbox' id='$id' name='" . $fanoe_option_name . "[$id]' value='1' " . checked( $options[$id], 1, false ) . " />";
			echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : "";
		break;
		...
		case "multi-checkbox":
			foreach($choices as $item) {
				...
				echo "<input class='checkbox$field_class' type='checkbox' id='$id|$item[1]' name='" . $fanoe_option_name . "[$id|$item[1]]' value='1' $checked /> $item[0] <br/>";
			}
			echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : "";
		break;
	}
 ...
 * @return echoes output
 */
function fanoe_settings_page_fn() {
// get the settings sections array
	$settings_output = fanoe_get_settings();
?>
	<div class="wrap">
		<div class="icon32" id="icon-options-general"></div>
		<h2><?php echo $settings_output['fanoe_page_title']; ?></h2>
		
		<form action="options.php" method="post">
			<?php 
			// http://codex.wordpress.org/Function_Reference/settings_fields
			settings_fields($settings_output['fanoe_option_name']); 
			// http://codex.wordpress.org/Function_Reference/do_settings_sections
			do_settings_sections(__FILE__); 
			?>
			<p class="submit">
				<input name="Submit" type="submit" class="button-primary" value="<?php esc_attr_e('Save Changes','fanoe'); ?>" />
			</p>
 ...
 * @return array
 */
function fanoe_validate_options($input) {
	...
	// collect only the values we expect and fill the new $valid_input array i.e. whitelist our option IDs
		...
		$options = $settings_output['fanoe_page_fields'];
					...
					switch ( $option['class'] ) {
						...
						case 'numeric':
							...
							if(is_numeric($input[$option['id']]) == FALSE) {
								add_settings_error(
									...
									fanoe_SHORTNAME . '_txt_numeric_error', // error ID
									__('Expecting a Numeric value! Please fix.','fanoe'), // error message
						...
						case 'multinumeric':
							...
							if($input[$option['id']] !=''){
								...
								$valid_input[$option['id']] = (preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) == 1) ? $input[$option['id']] : __('Expecting comma separated numeric values','fanoe');
							}else{
							...
							if($input[$option['id']] !='' && preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) != 1) {
								add_settings_error(
									$option['id'], // setting title
									fanoe_SHORTNAME . '_txt_multinumeric_error', // error ID
									__('Expecting comma separated numeric values! Please fix.','fanoe'), // error message
									'error' // type of message
								);
							}
						break;
						...
						case 'email':
							...
							$input[$option['id']] 		= trim($input[$option['id']]); // trim whitespace
							if($input[$option['id']] != ''){
								$valid_input[$option['id']] = (is_email($input[$option['id']])!== FALSE) ? $input[$option['id']] : __('Invalid email! Please re-enter!','fanoe');
							}elseif($input[$option['id']] == ''){
								$valid_input[$option['id']] = __('This setting field cannot be empty! Please enter a valid email address.','fanoe');
							}
							...
							if(is_email($input[$option['id']])== FALSE || $input[$option['id']] == '') {
								add_settings_error(
									$option['id'], // setting title
									fanoe_SHORTNAME . '_txt_email_error', // error ID
									__('Please enter a valid email address.','fanoe'), // error message
									'error' // type of message
								);
							}
						break;
 ...
 */
function fanoe_show_msg($message, $msgclass = 'info') {
...
function fanoe_admin_msgs() {
	...
	if(current_user_can ('manage_options') && $fanoe_settings_pg !== FALSE && !empty($set_errors)){
		...
		if($set_errors[0]['code'] == 'settings_updated' && isset($_GET['settings-updated'])){
			fanoe_show_msg("<p>" . $set_errors[0]['message'] . "</p>", 'updated');
...
// admin messages hook!
add_action('admin_notices', 'fanoe_admin_msgs');
